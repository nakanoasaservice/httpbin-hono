 ...
 * Note: This must be called before getRawData or getBody, as it consumes the request body
 */
export async function getFiles(
	...
	try {
		...
		for (const [key, value] of Object.entries(formData)) {
			// Hono's parseBody may return File objects
...
/**
 * Get form data from multipart/form-data (non-file fields only)
 * Note: This must be called before getRawData, as it consumes the request body
 */
export async function getFormFromMultipart(
	c: Context,
...
export async function getBody(c: Context): Promise<unknown> {
	const contentType = c.req.header("content-type") || "";
	...
	if (contentType.includes("application/json")) {
		...
		try {
			const formData = await c.req.parseBody();
			return semiflatten(formData as Record<string, string | string[]>);
		} catch {
			return null;
		}
...
/**
 * Try to parse request body as JSON, regardless of Content-Type
 */
export async function tryParseJson(c: Context): Promise<unknown> {
	try {
